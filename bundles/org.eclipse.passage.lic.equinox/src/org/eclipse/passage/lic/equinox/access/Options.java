...
final class Options<D extends Enum<?>> {
	...
	Option<D> promptAndPick() {
		while (true) {
			...
			// Bypass input prompt in FLS console, enabling immediate start-up without user input
			// "p" means "Proceed"
			// TODO: Find out where the console menu of the FLS is implemented and disable it directly
			String key = "p"; //$NON-NLS-1$ 
			// interaction.input().trim();
